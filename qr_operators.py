...
from .setup import InstallQuadRemesherEngine
...
def InstallQuadRemesherEngineIfNeeded(op, context, enginePath):
    ...
    if not os.path.exists(enginePath):
        console_print(t("Engine not found: downloading and installing...("+enginePath+")"))
        ...
        if downloadOK == False:
            if downloadExceptionCount > 0:
                console_print(t(downloadExceptionText))
        ...
        try:
            ...
            import traceback
            console_print(t("Install remesher Engine ERROR: Unzip Failed" + str(traceback.format_exc()) + "\n"))
...
def doRemeshing_Start(theOp, context) :
    ...
    if export_selected_mesh_fbx(inputFilename) == False:
        theOp.report({'ERROR'}, t("Cannot use FBX exporter. Please make sure the FBX format addon is enabled."))
        return 
    ...
    def modal(self, context, event):  
        ...
        if event.type == 'TIMER':
            ...
            if ProgressValueFloat >= 0 and ProgressValueFloat <= 1.0:
                ...
                self.report({'INFO'}, t("Remeshing progress:"+str(newPBarValue)+" % (ESC=Abort)"))
            else:
...
class QREMESHER_OT_license_manager(bpy.types.Operator):
    ...
    bl_description = t("Launches the License manager")
    bl_options = {'REGISTER'}
...
class QREMESHER_OT_facemap_to_materials(bpy.types.Operator):
    ...
    bl_description = t("Assign new Materials to each FaceMap (using random colors) so that:\n- FaceMaps can be used with 'Use Materials' option.\n- FaceMaps can be easily visualized.")
    bl_options = {'REGISTER', 'UNDO'}
...
class QREMESHER_OT_News_LatestVer(bpy.types.Operator):
    ...
    def readNews(cls):
        ...
        cls.newsButtonLabel = t("Check Version")
        cls.newsButtonDesc = t("Check Version description")
